...
class NodeMinmaxEvaluation:
    ...
    best_move_sequence: list[BranchKey] = field(
    ...
    moves_sorted_by_value_: dict[BranchKey, Any] = field(
        default_factory=lambda: dict[BranchKey, Any]()
    )
    ...
    moves_not_over: list[BranchKey] = field(default_factory=lambda: list[BranchKey]())

    # creating a base Over event that is set to None
    over_event: OverEvent = field(default_factory=OverEvent)

    @property
    def moves_sorted_by_value(self) -> dict[BranchKey, Any]:
        """
    ...
    def subjective_value_(self, value_white: float) -> float:
        ...
        The subjective value is calculated based on the player to move. If the player to move is `Color.WHITE`, then the
    ...
    def subjective_value(self) -> float:
        ...
        If the player to move is Color.WHITE, the subjective value is self.get_value_white().
        If the player to move is not Color.WHITE, the subjective value is -self.get_value_white().
    ...
    def best_move(self) -> BranchKey | None:
        """
    ...
    def best_move_not_over(self) -> BranchKey:
        """
        ...
        for branch_key in self.moves_sorted_by_value:
            child = self.tree_node.moves_children[branch_key]
    ...
    def second_best_move(self) -> BranchKey:
        """
        ...
        second_best_branch: BranchKey = nth_key(self.moves_sorted_by_value, 1)
    ...
    def print_moves_sorted_by_value(self) -> None:
        ...
        branch_key: BranchKey
        for branch_key, subjective_sort_value in self.moves_sorted_by_value.items():
            print(
                self.tree_node.board.get_uci_from_move_key(branch_key),
    ...
    def print_moves_sorted_by_value_and_exploration(self) -> None:
        ...
        string_info: str = ""
        for branch_key, subjective_sort_value in self.moves_sorted_by_value.items():
            string_info += f" {self.tree_node.board.get_uci_from_move_key(branch_key)} {subjective_sort_value[0]} $$ "
    ...
    def record_sort_value_of_child(self, branch_key: BranchKey) -> None:
        ...
        if self.is_over():
            ...
            self.moves_sorted_by_value_[branch_key] = (
                subjective_value_of_child,
    ...
    def becoming_over_from_children(self) -> None:
        ...
        branch_key: BranchKey
        for branch_key in self.tree_node.moves_children:
            self.record_sort_value_of_child(branch_key=branch_key)

        # fast way to access first key with the highest subjective value
        best_branch_key: BranchKey | None = self.best_move()
        assert best_branch_key is not None
        best_child = self.tree_node.moves_children[best_branch_key]
    ...
    def update_over(self, moves_with_updated_over: set[BranchKey]) -> bool:
        """
        Update the over_event of the node based on notification of change of over_event in children.

        Args:
            moves_with_updated_over (set[BranchKey]): A set of branch keys linking to the children
    ...
    def update_moves_values(self, moves_to_consider: set[BranchKey]) -> None:
        """
        Updates the values of the moves based on the given set of moves to consider.

        Args:
            moves_to_consider (set[BranchKey]): The set of moves to consider.

        Returns:
            None
        """
        for branch_key in moves_to_consider:
            self.record_sort_value_of_child(branch_key=branch_key)
        self.moves_sorted_by_value_ = sort_dic(self.moves_sorted_by_value_)

    def sort_moves_not_over(self) -> list[BranchKey]:
        """
    ...
    def update_best_move_sequence(
        self, moves_with_updated_best_move_seq: set[BranchKey]
    ) -> bool:
        ...
        best_branch_key: BranchKey = self.best_move_sequence[0]
        best_node: NodeWithValue | None = self.tree_node.moves_children[best_branch_key]

        if (
            best_branch_key in moves_with_updated_best_move_seq
    ...
    def one_of_best_children_becomes_best_next_node(self) -> None:
        ...
        best_moves: list[BranchKey] = self.get_all_of_the_best_moves(
            how_equal=how_equal_
        )
        if how_equal_ == "equal":
            assert len(best_moves) == 1
        best_branch_key = choice(best_moves)
        best_child = self.tree_node.moves_children[best_branch_key]
    ...
    def minmax_value_update_from_children(
        self, moves_with_updated_value: set[BranchKey]
    ) -> tuple[bool, bool]:
        """
        ...
        best_branch_key_before_update: BranchKey | None = self.best_move()
        ...
        best_move_seq_before_update: list[BranchKey] = self.best_move_sequence.copy()
        if self.tree_node.all_legal_moves_generated:
    ...
    def description_best_move_sequence(self) -> str:
        ...
        res = ""
        branch_key: BranchKey
        for branch_key in self.best_move_sequence:
    ...
    def test_children_not_over(self) -> None:
        ...
        for branch_key, child in self.tree_node.moves_children.items():
            assert child is not None
            if child.is_over():
                assert branch_key not in self.moves_not_over
            else:
                assert branch_key in self.moves_not_over
    ...
    def get_all_of_the_best_moves(
        ...
        """
        best_moves: list[BranchKey] = []
        best_move: BranchKey | None = self.best_move()
        ...
        for branch_key in self.moves_sorted_by_value:
            if how_equal == "equal":
                if self.are_equal_values(
                    self.moves_sorted_by_value[branch_key], best_value
                ):
                    best_moves.append(branch_key)
                    assert len(best_moves) == 1
    ...
    def evaluate(self) -> BoardEvaluation:
        if self.over_event.is_over():
            return ForcedOutcome(
                ...
                line=[branch_key for branch_key in self.best_move_sequence],
            )
        else:
