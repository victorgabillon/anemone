...
import chess
import yaml
from atomheart import create_board_chi_from_pychess_board
...
from anemone.trees.value_tree import (
    ValueTree,
)
...
def make_tree_from_file(
    ...
    half_moves: dict[int, TreeDepth] = {}
    id_nodes: dict[int, AlgorithmNode] = {}
    move_and_value_tree: ValueTree | None = None
    ...
    for yaml_node in yaml_nodes:
        if yaml_node["id"] == 0:
            ...
            board_chi = ChessBoardTest(board=board)
...
def check_from_file(file_path: path, tree: ValueTree) -> None:
    """
...
def check_index(index_computation: IndexComputationType, tree_file: path) -> TestResult:
    ...
    tree: ValueTree = make_tree_from_file(
